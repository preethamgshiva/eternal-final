...
function ModelInner({
  ...
  // NOTE: useGLTF, useFBX and useLoader are hooks — calling them inside useMemo is not ideal,
  // but to keep your original behavior and avoid a large refactor we keep the previous pattern.
  // If you refactor later you may want a dedicated loader component per format.
  const content = useMemo<THREE.Object3D | null>(() => {
    ...
    console.error("Unsupported or failed to load format:", ext);
    return null;
  }, [url, ext]);
    ...
    >
      ...
      <Canvas
        ...
        onCreated={({ gl, scene, camera }) => {
          ...
          // Note: outputColorSpace requires three r125+, adjust if using older three
          // @ts-ignore
